...
/**
 * Unload resources used by transformation.
 * <p>
 * Unlike {@link FinalizeSubsystemTransitionActivity}, this file does save the result. <br/>
 * Saving result is {@link FinalizeSubsystemTransitionActivity} responsibility.
 * </p>
 * 
 * @author nperansin@obeo.fr
 */
public class UnloadSubsystemResourcesActivity extends AbstractActivity {

  public static final String ID = UnloadSubsystemResourcesActivity.class.getName(); //$NON-NLS-1$
  ...
  private void unloadTransformationProject(Resource transformationResource) {
  ...
  private void unloadTemporaryResource(ActivityParameters activityParams_p) {
    ...
    if (transformationRoot != null) {
      ...
      // inter and crossphase transform into a resourceless model, 
      // so that in these cases the transformationResource is null
      if (transformationResource != null && transformationResource != targetResource) {
        unloadTransformationProject(transformationResource);
      }

      // Selected target file is loaded into
      // the editing domain of the source resource to process the transformation.
      // now we're done with it and need to unload it.
      if (targetResource != null) {
        
        ResourceSet set = targetResource.getResourceSet();
        for (Resource resource : SsbsFinalizeTransitionActivity.getProjectRelatedResources(targetResource)) {
          resource.unload();
  ...
  private void handleTemporaryProject(ActivityParameters activityParams_p) {
    if (!keepTransformationProject()) {
      IProject toDelete = ((IProject) getContext(activityParams_p).get(FinalizeSubsystemTransitionActivity.PARAM__DELETE_PROJECT));
      if (toDelete != null) {
        try {
